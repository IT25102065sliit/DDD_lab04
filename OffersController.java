...
public class OffersController {
    ...
    private void handleAdd() {
        ...
        try (Connection conn = Database.getConnection();
            ...
            stmt.setInt(3, Integer.parseInt(txtYear.getText().trim()));
    ...
    private void handleDelete() {
        String sql = "DELETE FROM Offers WHERE CID = ? AND Mcode = ? AND Academic_year = ? AND Semester = ?";
        try (Connection conn = Database.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            lblStatus.setText(rows > 0 ? "Offer mapping deleted." : "No matching offer record found.");
